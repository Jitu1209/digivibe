        ...
        <div className="grid grid-cols-1 md:grid-cols-5 gap-8 items-start">
            ...
            <div className="glass-panel p-6 rounded-2xl border border-white/10">
              ...
              <form onSubmit={handleStartPayment} className="space-y-4">
                <div>
                  ...
                  <input type="text" required value={form.name} onChange={(e) => setForm({...form, name: e.target.value})} className="w-full bg-white border border-white/10/80 shadow-sm border border-white/10 rounded-lg px-3.5 py-2.5 text-xs text-slate-800 focus:border-brand-purple focus:ring-1 focus:ring-brand-purple outline-none shadow-sm focus:border-brand-purple outline-none" placeholder="Enter Full Name" />
                </div>
                <div>
                  <label className="block text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-1.5">Email Address *</label>
                  <input type="email" required value={form.email} onChange={(e) => setForm({...form, email: e.target.value})} className="w-full bg-white border border-white/10/80 shadow-sm border border-white/10 rounded-lg px-3.5 py-2.5 text-xs text-slate-800 focus:border-brand-purple focus:ring-1 focus:ring-brand-purple outline-none shadow-sm focus:border-brand-purple outline-none" placeholder="enteremail@gmail.com" />
                </div>
                <div>
                  <label className="block text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-1.5">Phone Number *</label>
                  <input type="tel" required value={form.phone} onChange={(e) => setForm({...form, phone: e.target.value})} className="w-full bg-white border border-white/10/80 shadow-sm border border-white/10 rounded-lg px-3.5 py-2.5 text-xs text-slate-800 focus:border-brand-purple focus:ring-1 focus:ring-brand-purple outline-none shadow-sm focus:border-brand-purple outline-none" placeholder="+91 98765 43210" />
                </div>
                ...
                <button type="submit" className="w-full bg-gradient-to-r from-brand-purple to-brand-blue hover:brightness-110 text-black font-bold py-3.5 rounded-xl text-xs uppercase tracking-widest transition-all mt-6 flex items-center justify-center space-x-2">
          ...
          <div className="md:col-span-2 space-y-6">
            <div className="glass-panel p-6 rounded-2xl border border-white/10 space-y-4">
              ...
              <div className="border-t border-white/10/60 pt-4 flex items-center justify-between font-bold">
                <span className="text-xs text-gray-300 uppercase">Total Cost</span>
                <span className="text-lg text-white">₹{course.fee.toLocaleString()}</span>
              </div>
            </div>

            <div className="bg-brand-dark border border-white/10/60 p-4 rounded-xl flex items-center space-x-3 text-gray-400 text-xs">
      ...
      {showRazorpay && (
        <div className="fixed inset-0 z-50 overflow-y-auto bg-white/90 flex items-center justify-center p-4">
          <div className="bg-[#1C2036] border border-white/15 rounded-xl max-w-md w-full shadow-2xl overflow-hidden font-sans">
            ...
            <div className="bg-[#111425] p-5 flex items-center justify-between border-b border-white/10/60">
              <div className="flex items-center space-x-2">
                <div className="bg-[#3399FF] text-white p-1 rounded font-extrabold text-[10px] uppercase">
                  RP
                </div>
                <div>
              ...
              <div className="text-right">
                ...
                <span className="text-sm font-extrabold text-white">₹{course.fee.toLocaleString()}</span>
              </div>
            </div>
            ...
            <div className="p-6 space-y-6">
              ...
              {paymentLoading ? (
                ...
                <div className="py-12 text-center space-y-4">
                  ...
                  <p className="text-xs font-bold text-gray-300 uppercase tracking-widest">Processing Payment Transaction...</p>
                  <p className="text-[10px] text-gray-400">Contacting Razorpay Gateway (Secure Mock Portal)</p>
                </div>
              ) : (
                /* Payment Options */
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    <button type="button" onClick={() => setPaymentMethod('card')} className={`p-3 rounded-lg border text-center transition-all ${paymentMethod === 'card' ? 'border-[#3399FF] bg-[#3399FF]/10 text-white' : 'border-white/10/60 bg-brand-darker text-gray-400'}`}>
                      <CreditCard className="w-5 h-5 mx-auto mb-1 text-center" />
                      <span className="text-[9px] uppercase font-bold">Card</span>
                    </button>
                    <button type="button" onClick={() => setPaymentMethod('upi')} className={`p-3 rounded-lg border text-center transition-all ${paymentMethod === 'upi' ? 'border-[#3399FF] bg-[#3399FF]/10 text-white' : 'border-white/10/60 bg-brand-darker text-gray-400'}`}>
                      <Sparkles className="w-5 h-5 mx-auto mb-1" />
                      <span className="text-[9px] uppercase font-bold">UPI / GPay</span>
                    </button>
                    <button type="button" onClick={() => setPaymentMethod('net')} className={`p-3 rounded-lg border text-center transition-all ${paymentMethod === 'net' ? 'border-[#3399FF] bg-[#3399FF]/10 text-white' : 'border-white/10/60 bg-brand-darker text-gray-400'}`}>
                      <Lock className="w-5 h-5 mx-auto mb-1" />
                      <span className="text-[9px] uppercase font-bold">NetBank</span>
                    </button>
                  </div>

                  <div className="border border-white/10/60 rounded-xl p-4 bg-brand-darker space-y-3 text-xs">
                    {paymentMethod === 'card' && (
                      <div className="space-y-2">
                        <label className="text-[9px] font-bold text-gray-400 uppercase">Mock Card Number</label>
                        <input type="text" readOnly className="w-full bg-white border border-white/10/80 shadow-sm border border-white/10 rounded px-2.5 py-1.5 font-mono text-gray-300 text-xs" value="4111 2222 3333 4444" />
                        <div className="grid grid-cols-2 gap-2 mt-2">
                          <div>
                            <label className="text-[9px] font-bold text-gray-400 uppercase">Expiry</label>
                            <input type="text" readOnly className="w-full bg-white border border-white/10/80 shadow-sm border border-white/10 rounded px-2.5 py-1.5 text-gray-300 text-xs text-center" value="12/28" />
                          </div>
                          <div>
                            <label className="text-[9px] font-bold text-gray-400 uppercase">CVV</label>
                            <input type="password" readOnly className="w-full bg-white border border-white/10/80 shadow-sm border border-white/10 rounded px-2.5 py-1.5 text-gray-300 text-xs text-center" value="123" />
                          </div>
                        </div>
                      </div>
                    )}
                    ...
                    {paymentMethod === 'upi' && (
                      <div className="space-y-2">
                        <label className="text-[9px] font-bold text-gray-400 uppercase">UPI Virtual Address</label>
                        <input type="text" readOnly className="w-full bg-white border border-white/10/80 shadow-sm border border-white/10 rounded px-2.5 py-1.5 font-mono text-gray-300 text-xs" value="student@okhdfcbank" />
                      </div>
                    )}

                    {paymentMethod === 'net' && (
                      <div className="space-y-2 text-center py-2 text-gray-400 text-[11px]">
                  ...
                  <div className="bg-[#EE534F]/10 border border-[#EE534F]/20 p-3 rounded-lg text-[10px] text-gray-300 leading-relaxed text-justify">
                  ...
                  <div className="flex space-x-3 pt-4">
                    <button type="button" onClick={() => setShowRazorpay(false)} className="w-1/3 bg-brand-darker border border-white/10 hover:bg-white/10 text-white font-bold py-2.5 rounded-lg text-xs uppercase">
                      Cancel
                    </button>
                    <button type="button" onClick={handleCompleteMockPayment} className="w-2/3 bg-[#3399FF] hover:bg-[#2288EE] text-white font-bold py-2.5 rounded-lg text-xs uppercase tracking-wider shadow-lg shadow-[#3399FF]/20">
                      Pay Mock Amount
                    </button>
                  </div>
                </div>
            ...
            {/* Footer */}
            <div className="bg-[#111425] p-3 text-center border-t border-white/10/60 text-[9px] text-gray-400 uppercase tracking-widest flex items-center justify-center space-x-1">
