  ...
  const handleStartPayment = async (e) => {
    ...
    try {
      ...
      const response = await fetch('/api/create-order', {
        ...
        body: JSON.stringify({
          amount: course.fee * 100, // in paise
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.error || 'Failed to create payment order on the server.');
      }
    ...
    const options = {
      ...
      image: '/favicon.svg',
      order_id: orderData.order_id, // Pass order_id generated by backend
      prefill: {
        name: form.name,
        email: form.email,
        contact: form.phone,
      ...
      handler: async function (response) {
        try {
          ...
          alert(`Payment Verification Failed: ${err.message}. Please contact support if amount was deducted.`);
          setPaymentLoading(false);
        }
      },
      modal: {
