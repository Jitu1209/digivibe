      ...
      {showRazorpay && (
        <div className="fixed inset-0 z-50 overflow-y-auto bg-black/90 flex items-center justify-center p-4">
          <div className="bg-[#1C2036] border border-white/15 rounded-xl max-w-md w-full shadow-2xl overflow-hidden font-sans">
            ...
            <div className="bg-[#111425] p-5 flex items-center justify-between border-b border-white/5">
            ...
            <div className="p-6 space-y-6">
              ...
              {paymentLoading ? (
                ...
                <div className="py-12 text-center space-y-4">
                  ...
                  <p className="text-[10px] text-gray-500">Contacting Razorpay Gateway (Secure Mock Portal)</p>
                </div>
              ) : (
                /* Payment Options */
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    <button type="button" onClick={() => setPaymentMethod('card')} className={`p-3 rounded-lg border text-center transition-all ${paymentMethod === 'card' ? 'border-[#3399FF] bg-[#3399FF]/10 text-white' : 'border-white/5 bg-white/5 text-gray-400'}`}>
                      <CreditCard className="w-5 h-5 mx-auto mb-1 text-center" />
                      <span className="text-[9px] uppercase font-bold">Card</span>
                    </button>
                    <button type="button" onClick={() => setPaymentMethod('upi')} className={`p-3 rounded-lg border text-center transition-all ${paymentMethod === 'upi' ? 'border-[#3399FF] bg-[#3399FF]/10 text-white' : 'border-white/5 bg-white/5 text-gray-400'}`}>
                      <Sparkles className="w-5 h-5 mx-auto mb-1" />
                      <span className="text-[9px] uppercase font-bold">UPI / GPay</span>
                    </button>
                    <button type="button" onClick={() => setPaymentMethod('net')} className={`p-3 rounded-lg border text-center transition-all ${paymentMethod === 'net' ? 'border-[#3399FF] bg-[#3399FF]/10 text-white' : 'border-white/5 bg-white/5 text-gray-400'}`}>
                      <Lock className="w-5 h-5 mx-auto mb-1" />
                      <span className="text-[9px] uppercase font-bold">NetBank</span>
                    </button>
                  </div>

                  <div className="border border-white/5 rounded-xl p-4 bg-white/5 space-y-3 text-xs">
                    {paymentMethod === 'card' && (
                      <div className="space-y-2">
                        <label className="text-[9px] font-bold text-gray-400 uppercase">Mock Card Number</label>
                        <input type="text" readOnly className="w-full bg-black/40 border border-white/10 rounded px-2.5 py-1.5 font-mono text-gray-300 text-xs" value="4111 2222 3333 4444" />
                        <div className="grid grid-cols-2 gap-2 mt-2">
                          <div>
                            <label className="text-[9px] font-bold text-gray-400 uppercase">Expiry</label>
                            <input type="text" readOnly className="w-full bg-black/40 border border-white/10 rounded px-2.5 py-1.5 text-gray-300 text-xs text-center" value="12/28" />
                          </div>
                          <div>
                            <label className="text-[9px] font-bold text-gray-400 uppercase">CVV</label>
                            <input type="password" readOnly className="w-full bg-black/40 border border-white/10 rounded px-2.5 py-1.5 text-gray-300 text-xs text-center" value="123" />
                          </div>
                        </div>
                      </div>
                    )}
                    ...
                    {paymentMethod === 'upi' && (
                      <div className="space-y-2">
                        ...
                        <input type="text" readOnly className="w-full bg-black/40 border border-white/10 rounded px-2.5 py-1.5 font-mono text-gray-300 text-xs" value="student@okhdfcbank" />
                      </div>
                    )}
                  ...
                  <div className="flex space-x-3 pt-4">
                    <button type="button" onClick={() => setShowRazorpay(false)} className="w-1/3 bg-white/5 border border-white/10 hover:bg-white/10 text-white font-bold py-2.5 rounded-lg text-xs uppercase">
                      Cancel
                    </button>
            ...
            {/* Footer */}
            <div className="bg-[#111425] p-3 text-center border-t border-white/5 text-[9px] text-gray-500 uppercase tracking-widest flex items-center justify-center space-x-1">
