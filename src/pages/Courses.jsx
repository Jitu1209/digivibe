        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 z-10 relative">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-12 items-start mb-16">
            ...
            <div className="glass-panel p-6 rounded-2xl border border-white/10 text-center">
              <span className="text-xs text-gray-400 uppercase font-bold tracking-widest">Certification Cost</span>
              <p className="text-3xl sm:text-4xl font-extrabold text-white mt-2 mb-1">
                ₹{course.fee.toLocaleString()}
              </p>
              <p className="text-[10px] text-gray-400 mb-6">Inclusive of recorded lectures, live mentor reviews & digital badge.</p>
              
              <button onClick={() => handleEnroll(course.id)} className="w-full bg-gradient-to-r from-brand-purple to-brand-blue hover:brightness-110 text-black font-bold py-3.5 rounded-xl text-xs uppercase tracking-widest transition-all transform hover:scale-[1.02]">
                Enroll & Onboard Now
              </button>

              {/* Prominent disclaimer under checkouts */}
              <div className="mt-6 pt-6 border-t border-white/10/60 flex items-start space-x-2 text-[10px] text-gray-400 text-left leading-normal bg-brand-darker p-3.5 rounded-xl">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-16 items-start">
              ...
              <div>
                ...
                <div className="space-y-4">
                  {course.curriculum.map((module, idx) => (
                    <div key={idx} className="bg-brand-darker border border-white/10/60 p-5 rounded-xl flex items-start space-x-4">
                      ...
                      <div>
                        <h4 className="font-bold text-white text-sm sm:text-base">{module.title}</h4>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              <div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                  {course.mentors.map((mentor, idx) => (
                    <div key={idx} className="bg-brand-darker border border-white/10/60 p-5 rounded-xl flex items-start space-x-3">
                      ...
                      <div>
                        ...
                        <p className="text-[10px] text-gray-400 mt-0.5">{mentor.experience} Experience</p>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              <div>
                ...
                <div className="space-y-4">
                  {course.faqs.map((faq, idx) => (
                    <div key={idx} className="border border-white/10/60 p-5 rounded-xl">
                      <h4 className="font-bold text-white text-xs sm:text-sm mb-2">{faq.q}</h4>
                      <p className="text-[11px] sm:text-xs text-gray-400 leading-relaxed">{faq.a}</p>
                    </div>
                  ))}
                </div>
              </div>
            ...
            <div className="space-y-6 sticky top-24">
              ...
              <div className="bg-brand-darker border border-white/10/60 p-6 rounded-2xl">
                <h4 className="text-xs font-bold text-white uppercase tracking-wider mb-2">Student Reviews</h4>
                <div className="space-y-4 mt-4">
                  {course.reviews.map((rev, idx) => (
                    <div key={idx} className="border-b border-white/10/60 pb-4 last:border-b-0 last:pb-0">
  ...
  return (
    <div className="text-white min-h-screen relative pt-12 pb-24">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 z-10 relative">
        ...
        <div className="flex flex-col md:flex-row items-center justify-between gap-6 mb-12 bg-brand-darker border border-white/10 p-6 rounded-2xl">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {filteredCourses.map((c) => (
            <div key={c.id} className="glass-card p-6 rounded-2xl flex flex-col justify-between border border-white/10/60 hover:border-brand-purple/20 transition-all hover:scale-[1.01]">
              <div>
              ...
              <div>
                <div className="flex items-center space-x-1 mb-4 text-brand-cyan">
                  ...
                  <span className="text-[10px] text-gray-400">({c.enrollments})</span>
                </div>
                ...
                <div className="border-t border-white/10/60 pt-4 flex items-center justify-between">
                  ...
                  <div className="flex space-x-2">
                    <button onClick={() => handleViewCourse(c.id)} className="bg-brand-darker hover:bg-white/10 border border-white/10 text-white font-bold text-xs uppercase px-3 py-2.5 rounded-lg transition-colors">
                      Syllabus
                    </button>
