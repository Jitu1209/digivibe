      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 z-10 relative">
        ...
        <div className="flex flex-wrap justify-center gap-3 mb-12">
          {categories.map((cat, idx) => (
            <button key={idx} onClick={() => setSelectedCategory(cat)} className={`px-5 py-2 rounded-full font-bold text-xs uppercase tracking-wider transition-all ${selectedCategory === cat ? 'bg-brand-purple text-black' : 'bg-brand-darker border border-white/10 text-gray-300 hover:text-white'}`}>
              {cat}
            </button>
          ))}
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {filteredBlogs.map((blog) => (
            <div key={blog.id} className="glass-card rounded-2xl overflow-hidden flex flex-col justify-between border border-white/10/60 hover:border-brand-purple/20 transition-all hover:scale-[1.01]">
              <div>
                <img src={blog.image} alt={blog.title} className="w-full h-48 object-cover border-b border-white/10/60" />
                <div className="p-6">
                  ...
                  <h3 className="text-lg font-bold text-white mb-2 line-clamp-2 h-14">{blog.title}</h3>
                  <p className="text-xs text-gray-400 leading-relaxed line-clamp-3">{blog.summary}</p>
                </div>
              </div>
        ...
        {selectedBlog && (
          <div className="fixed inset-0 z-50 overflow-y-auto bg-white/85 backdrop-blur-sm flex items-center justify-center p-4">
            <div className="bg-brand-dark border border-white/10 rounded-2xl max-w-3xl w-full max-h-[85vh] overflow-y-auto shadow-2xl relative">
              <button onClick={() => setSelectedBlog(null)} className="absolute top-4 right-4 bg-white/60 hover:bg-white p-2 rounded-full border border-white/10 text-white z-10">
                <X className="w-5 h-5" />
              </button>
              
              <img src={selectedBlog.image} alt={selectedBlog.title} className="w-full h-64 object-cover border-b border-white/10" />
              ...
              <div className="p-8">
                <div className="flex items-center space-x-4 mb-4 text-xs">
                  ...
                  <span className="text-gray-400 font-mono">By {selectedBlog.author}</span>
                </div>
                
                <h2 className="logo-font text-2xl sm:text-3xl font-extrabold text-white mb-6">
                  {selectedBlog.title}
                </h2>
                ...
                <div className="mt-8 pt-6 border-t border-white/10/60 flex items-center justify-between text-xs text-gray-400">
