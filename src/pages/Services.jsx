...
export default function Services() {
  ...
  if (!data) {
    return (
      <div className="text-white min-h-[60vh] flex flex-col items-center justify-center p-6 text-center">
        ...
        <Link to="/" className="bg-brand-orange text-black font-bold px-6 py-2.5 rounded-lg text-xs uppercase tracking-wider">
          Return Home
        </Link>
      </div>
    );
  ...
  return (
    <div className="text-white min-h-screen relative pt-12 pb-24">
      {/* Decorative Blur */}
      <div className="absolute top-20 right-1/4 w-96 h-96 bg-brand-orange/5 rounded-full blur-[100px] animate-pulse-glow z-0"></div>

      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 z-10 relative">
        {/* Back Link */}
        <Link to="/" className="inline-flex items-center space-x-2 text-xs font-bold text-gray-400 hover:text-brand-orange uppercase tracking-wider mb-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-16 items-start">
          ...
          <div className="lg:col-span-2 space-y-12">
            ...
            {/* Tech Stack */}
            <div>
              <h3 className="text-lg font-bold text-white uppercase tracking-wider mb-4 border-l-2 border-brand-orange pl-3">
                Technologies & Platforms
              </h3>
              <div className="flex flex-wrap gap-2.5">
                {data.technologies.map((t, idx) => (
                  <span key={idx} className="text-xs bg-white/5 border border-white/10 px-3 py-1.5 rounded-lg text-gray-300 font-mono">
                    {t}
                  </span>
                ))}
              </div>
            ...
            {/* Process Timeline */}
            <div>
              <h3 className="text-lg font-bold text-white uppercase tracking-wider mb-8 border-l-2 border-brand-orange pl-3">
                Our Work Process
              </h3>
              <div className="relative border-l border-white/10 pl-6 space-y-8">
                {data.process.map((p, idx) => (
                  <div key={idx} className="relative">
                    <span className="absolute -left-[37px] bg-brand-orange text-black font-extrabold text-[10px] w-6 h-6 rounded-full flex items-center justify-center border-2 border-black">
                      {p.step}
                    </span>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-8">
              <div className="glass-panel p-6 rounded-xl border border-white/5">
                <h4 className="font-bold text-sm text-brand-orange uppercase tracking-wider mb-4">Core Deliverables</h4>
                <ul className="space-y-3 text-xs sm:text-sm text-gray-300">
                  {data.deliverables.map((item, idx) => (
                    <li key={idx} className="flex items-start space-x-2">
                      <Terminal className="w-4 h-4 text-brand-orange mt-0.5 flex-shrink-0" />
                      <span>{item}</span>
                    </li>
                  ))}
                </ul>
              ...
              <div className="glass-panel p-6 rounded-xl border border-white/5">
                <h4 className="font-bold text-sm text-brand-orange uppercase tracking-wider mb-4">Core Benefits</h4>
                <ul className="space-y-3 text-xs sm:text-sm text-gray-300">
            ...
            <div>
              ...
              <div className="space-y-4">
                {data.faqs.map((faq, idx) => (
                  <div key={idx} className="border border-white/5 bg-white/5 p-5 rounded-xl">
          ...
          <div className="glass-panel p-6 rounded-2xl border border-white/10 sticky top-24">
            ...
            {status === 'success' && (
              <div className="bg-brand-orange/15 border border-brand-orange/30 text-white p-3.5 rounded-lg mb-6 text-xs leading-relaxed">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                ...
                <input type="text" required value={form.name} onChange={(e) => setForm({...form, name: e.target.value})} className="w-full bg-black/40 border border-white/10 rounded-lg px-3.5 py-2.5 text-xs text-white focus:border-brand-orange focus:ring-1 focus:ring-brand-orange outline-none" placeholder="Jane Doe" />
              </div>
              <div>
                <label className="block text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-1.5">Company *</label>
                <input type="text" required value={form.company} onChange={(e) => setForm({...form, company: e.target.value})} className="w-full bg-black/40 border border-white/10 rounded-lg px-3.5 py-2.5 text-xs text-white focus:border-brand-orange focus:ring-1 focus:ring-brand-orange outline-none" placeholder="Company Name" />
              </div>
              <div>
                <label className="block text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-1.5">Email *</label>
                <input type="email" required value={form.email} onChange={(e) => setForm({...form, email: e.target.value})} className="w-full bg-black/40 border border-white/10 rounded-lg px-3.5 py-2.5 text-xs text-white focus:border-brand-orange focus:ring-1 focus:ring-brand-orange outline-none" placeholder="jane@company.com" />
              </div>
              <div>
                <label className="block text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-1.5">Phone *</label>
                <input type="tel" required value={form.phone} onChange={(e) => setForm({...form, phone: e.target.value})} className="w-full bg-black/40 border border-white/10 rounded-lg px-3.5 py-2.5 text-xs text-white focus:border-brand-orange focus:ring-1 focus:ring-brand-orange outline-none" placeholder="+91 98765 43210" />
              </div>
              <div>
                <label className="block text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-1.5">Budget *</label>
                <select value={form.budget} onChange={(e) => setForm({...form, budget: e.target.value})} className="w-full bg-black/40 border border-white/10 rounded-lg px-3.5 py-2.5 text-xs text-white focus:border-brand-orange focus:ring-1 focus:ring-brand-orange outline-none">
              ...
              <div>
                ...
                <textarea rows={3} value={form.message} onChange={(e) => setForm({...form, message: e.target.value})} className="w-full bg-black/40 border border-white/10 rounded-lg px-3.5 py-2.5 text-xs text-white focus:border-brand-orange focus:ring-1 focus:ring-brand-orange outline-none" placeholder="What are your goals?"></textarea>
              </div>
