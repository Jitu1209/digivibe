...
export default function About() {
  ...
  const timeline = [
    ...
    { year: '2026', title: 'Digivibe Integrated Workspace', desc: 'Deploying our integrated student onboarding flow, enabling PDF invoice tools and automatic project checks.' }
  ];
  ...
  return (
    <div className="text-white relative pt-12">
      ...
      {/* Decorative Glow */}
      <div className="absolute top-20 left-1/4 w-96 h-96 bg-brand-orange/10 rounded-full blur-[120px] animate-pulse-glow z-0"></div>
      ...
      <section className="relative px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto py-16 text-center z-10">
        ...
        <p className="mt-6 text-gray-400 max-w-3xl mx-auto text-sm sm:text-base leading-relaxed">
          Founded in 2022, Digivibe operates as a combined technology services organization. 
      ...
      <section className="py-16 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto z-10 relative">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
          <div className="space-y-6">
            ...
            <p className="text-gray-400 text-xs sm:text-sm leading-relaxed text-justify">
              Digivibe was established to resolve both. By hiring active programmers and digital advertisers to build real client projects inside our Agency vertical, and setting them up to instruct college students and career transitioners in our Academy vertical, we created a self-reinforcing practical learning environment.
            </p>
          </div>

          <div className="grid grid-cols-1 gap-6">
            <div className="glass-panel p-6 rounded-xl border border-white/10">
              <div className="flex items-center space-x-3 text-brand-orange mb-3">
          ...
          {coreValues.map((v, i) => {
            ...
            return (
              <div key={i} className="bg-white/5 border border-white/5 p-6 rounded-xl text-center">
                <div className="flex justify-center text-brand-orange mb-4">
        ...
        <div className="relative border-l border-white/10 pl-6 space-y-12">
          {timeline.map((item, idx) => (
            <div key={idx} className="relative">
              <div className="absolute -left-[31px] bg-brand-orange p-1 rounded-full text-black">
                <Calendar className="w-3 h-3" />
              </div>
              <div>
                <span className="font-mono text-xs font-bold text-brand-orange bg-brand-orange/10 px-2 py-0.5 rounded border border-brand-orange/30">
                  {item.year}
                </span>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-8">
          {MENTORS.map((m, idx) => (
            <div key={idx} className="text-center">
              ...
              <p className="text-xs text-brand-orange font-medium mt-0.5">{m.role}</p>
